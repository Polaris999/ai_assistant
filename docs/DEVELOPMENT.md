...
| 模块 | 说明 |
|------|------|
| `meeting_agent.agent_base` | Agent 协议：`AgentRunner`、`InvokeResult`、`is_agent_ready()`、`run_agent_warmup()` |
| `meeting_agent.api.response` | 统一响应：`code` / `msg` / `data`，`success()`、`json_response()` |
| `meeting_agent.api.middleware` | RequestID、安全头、请求日志 |
| `meeting_agent.api.agent_bootstrap` | `create_agent_or_placeholder(agent_factory=...)` |
| `meeting_agent.core` | exceptions、llm、embeddings、vectorstore、callbacks、helper |
| `meeting_agent.config` | Settings、校验、Prompt 加载 |

**统一入口**：`from meeting_agent.framework import ...` 可一次导入上述大部分符号（含中间件、InvokeResult、run_agent_warmup）。
...
1. **实现 AgentRunner**：新建模块（如 `meeting_agent.agent.xxx_agent`），实现 `invoke()` 与 `_scheduler`（可选 `is_ready()`、`warmup()`）；内部 `from meeting_agent.framework import get_llm, get_embeddings, ...`。
...
- **命令行**：安装后 `meeting-agent`，或 `python -m meeting_agent.main`（需 `PYTHONPATH=src` 或已安装）。
- **API**：根目录 `uvicorn app:app --reload`。
- **自检**：`meeting-agent --check-vector-store`、`meeting-agent --check-embedding` 检查向量库与 Embedding 可达性。
...
- **CLI**：`meeting-agent --check-config` 调用配置校验便于脚本检查。
